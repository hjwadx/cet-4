...
public class TranslateActivity extends BaseActivity{
	...
	private void initView() {
		...
		mViewPager.setOnPageChangeListener(new OnPageChangeListener() {
			...
			public void onPageSelected(int arg0) {
				...
				setTitle("翻译（" + (mViewPager.getCurrentItem() + 1) + "/" + mProblems.size() + "）");
				App.getInstance().setCurrentValue(Const.TRANSLATE_CURRENT, mPieces.get(mViewPager.getCurrentItem()).id);
			}
		...
		initTitlebar();
		Piece piece = mPieceMap.get(App.getInstance().getCurrentValue(Const.TRANSLATE_CURRENT));
	...
	private void initData(){
		mPieces = App.getInstance().getExamDBHelper().getPiecesByType(Problem.TRANSLATE);
