...
public class ListeningCompletenssItemView extends CompletenessItemView{
	
	private TextView mCompleteness;
	private Button mCommit;
	int mType;

	public ListeningCompletenssItemView(Context context, int type) {
		super(context);
		mType = type;
		setupViews();
	}

	public ListeningCompletenssItemView(Context context, AttributeSet attrs) {
		super(context, attrs);
		setupViews();
	}
	...
	public void setData( List<Problem> problems, HashMap<Integer, Piece> piece_map) {
		...
		mCommit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				saveResult();
				App.getInstance().setCurrentValue(Problem.getTypeCurrentString(mProblems.get(0).type), mPieceMap.get(mProblems.get(mProblems.size() - 1).piece_id).id);
				Intent intent = new Intent(getContext(), ListeningResultActivity.class);
	...
	void saveResult(){
		...
		for (Integer key : mPieceMap.keySet()) {
			pieces.add(mPieceMap.get(key));
		}
		App.getInstance().getDBHelper().savePieces(App.getInstance().getUserDB(), pieces);
